...
int main( int argc, char** argv ) {
  ...
  for(int i=0; i<CACHE_WARMUP; ++i){
    cpu::integral((float*)image.data, image.rows, image.cols, image.step1(), image.channels(),
  ...
  std::string outImagePath = std::string(std::getenv("OUTPUT")) + std::string(std::string("/integral_out.png"));
